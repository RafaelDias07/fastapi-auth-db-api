# Esse arquivo é responsavel por ter toda essa parte de gerenciar conexao com o banco, so que com as query dos clientes 
...
class ClienteRepositorio:
    ...
    async def obter_cliente(self, cliente_id: int) -> Cliente | None:
        with self.db.conectar() as conexao:
            ...
            linha = cursor.fetchone() # So precisamos da primeira linha
            if linha:
    ...
    async def criar_cliente(self, cliente: ClienteCriarAtualizar) -> Cliente:
        with self.db.conectar() as conexao:
            ...
            conexao.commit()
            cliente_id = cursor.lastrowid # toda vez que insere, ele consegue ler a ultima linha que foi criada no banco
    ...
    async def atualizar_cliente(self, cliente_id: int, cliente: ClienteCriarAtualizar) -> Cliente | None:
        with self.db.conectar() as conexao:
            ...
            if cursor.rowcount == 0: #Se nao encontrar nada quer dizer que não atualizou nada, assim nao precisa fazer nenhuma checkagem
                return None
            return Cliente(id_=cliente_id, nome=cliente.nome, email=cliente.email, telefone=cliente.telefone)
