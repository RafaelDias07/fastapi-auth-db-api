...
async def login(usuario_repositorio: Annotated[UsuarioRepositorio, Depends(obter_usuario_repositorio)], 
    ...
    if not all([email, senha, nome, confirma_senha]):  # Vai todos os campos de uma vez, sem precisar ficar validando 1 por 1
    ...
    return templates.TemplateResponse('registro.html', {
            'request': request,
            'error': 'Não foi possivel criar o usuário, tente novamente mais tarde!',
            **data
        })